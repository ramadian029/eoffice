...
<center>
    ...
    <br>
    <br>
    REKAPITULASI JUMLAH PENDUDUK MENINGGAL						
</center>
</b>
...
<table width="100%">
    <thead>
        <tr>
            ...
            <th align="center" colspan="6">JUMLAH PENDUDUK LAHIR MENINGGAL</th>
            <th align="center" rowspan="2" colspan="3">JUMLAH PENDUDUK MENINGGAL WNI + WNA</th>
...
//                    $data = $this->db->select(" a.* FROM data_kk a
//                                    INNER JOIN(
//                                    SELECT MAX(id_kk) AS id
//                                    FROM data_kk
//                                    WHERE bulan = '$periode'
...
//                                    ->where('a.id_kelurahan', $kel->id_kel)->get()->row();
                    ...
                    $wni_l = $this->db->select('COUNT(*) AS jml')
                                    ->from('data_mati')
                                    ->where('waktu >= ', $waktu_a)
                                    ->where('waktu < ', $waktu_b)
                    ...
                    $wni_p = $this->db->select('COUNT(*) AS jml')
                                    ->from('data_mati')
                                    ->where('waktu >= ', $waktu_a)
                                    ->where('waktu < ', $waktu_b)
...
//                    print_r($wni_p);
                    ...
                    $wna_l = $this->db->select('COUNT(*) AS jml')
                                    ->from('data_mati')
                                    ->where('waktu >= ', $waktu_a)
                                    ->where('waktu < ', $waktu_b)
                                    ->where('jenis_kelamin', 1)
                    ...
                    $wna_p = $this->db->select('COUNT(*) AS jml')
                                    ->from('data_mati')
                                    ->where('waktu >= ', $waktu_a)
                                    ->where('waktu < ', $waktu_b)
                                    ->where('jenis_kelamin', 2)
