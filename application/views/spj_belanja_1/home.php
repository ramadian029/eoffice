...
//        });
        ...
        $.ajax({
            url: "<?php echo site_url('spj_belanja/verifikasi') ?>?con=" + con + "&periode=" + periode + "&id_kelurahan=" + id_kelurahan,
            type: "POST",
            dataType: "JSON",
            error: function () {
    ...
    function ver_modal(con, periode, id_kelurahan) {
...
//        $(".btn_save").attr("id", $(this).attr('id'));
        $("#btn_save").attr("onclick", "verifikasi(" + con + ",'" + periode + "'," + id_kelurahan + ")");
                                        ...
                                        if ($jumlah > 0) {
                                            ...
                                            foreach ($data as $row) {
                                                ...
                                                <tr>
                                                    ...
                                                    <td><?php
                                                        $is_verified = $this->convertion->cek_verified($periode, md5($row->id_kelurahan));
                                                        if ($is_verified == 0) {
                                                        ...
                                                        <td align="center">
                                                            <?php if ($is_verified == 0) { ?>
                                                                <a class="btn btn-success btn-xs btn_verifikasi" href="#"  style="padding: 4px 8px; font-size: 12px"
                                                                   onclick="ver_modal('1', '<?= $periode ?>', '<?= $row->id_kelurahan ?>');">Verifikasi</a>
                                                               <?php } else if ($is_verified == 1) {
                                                                   ?>
                                                                <a class="btn btn-danger btn-xs btn_verifikasi" href="#"  style="padding: 4px 8px; font-size: 12px"
                                                                   onclick="ver_modal('0', '<?= $periode ?>', '<?= $row->id_kelurahan ?>');">Batalkan Verifikasi</a>
                                                                   <?php
                                                               }
                                                               ?>
                                                            &nbsp;
                                                            <a href='<?= site_url('spj_belanja/detail/' . md5($row->id_kelurahan) . '/' . $periode) ?>' style='color:#f0ad4e' data-target='tooltip' title='Detail'>
                                                                <i id="<?= md5($row->id_kelurahan) . '/' . $periode ?>" class='fa fa-file-text-o' style='color: #337ab7'></i></a>
                                                        </td>
                                                    <?php } ?>
                                                </tr>
                                                <?php $no++; ?>
