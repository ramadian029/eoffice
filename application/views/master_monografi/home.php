...
<section class="content">
    <div class="row">
        <div class="merah">DATA MONOGRAFI</div>
...
<!--                        <div class="col-lg-6">
                            <a href="<?= site_url('master_monografi/form') ?>">
                                    ...
                                    if ($jumlah > 0) {
                                        ...
                                        foreach ($data as $row) {
                                            ...
                                            <tr>
                                                ...
                                                <td align="center">
                <!--                                                    <a href="<?= site_url() ?>/master_monografi/form/<?= md5(sha1($row->id_pertanyaan)) ?>" class="edit"  title="Edit"><img src="<?= base_url('images') ?>/edit.png" style="width:15px;height:15px;"></a>&nbsp;&nbsp;&nbsp;
                                                            <a href="<?= site_url() ?>/master_monografi/delete/<?= md5(sha1($row->id_pertanyaan)) ?>" class="delete"  title="Delete" onclick="return confirm('Yakin hapus data?');"><img src="<?= base_url('images') ?>/delete.png" style="width:15px;height:15px;"></a>-->
                                                    <a href='<?= site_url('master_monografi/jawab/' . md5($row->id_laporan)) ?>'>
                                                        <span class="label label-success">ISI</span></a>&nbsp;&nbsp;&nbsp;
                                                    <a href='<?= site_url('master_monografi/hasil/' . md5($row->id_laporan)) ?>' 
                                                       style='color:#f0ad4e' data-target='tooltip' title='Hasil'>
                                                        <i id="<?= md5($row->id_laporan) ?>" class='fa fa-file-text-o' style='color: #337ab7'></i></a>
                                                    &nbsp;
                                                    <?php if ($_SESSION['level'] == '1') { ?>
            <!--                                                        <a href='<?= site_url('master_monografi/cetak/' . md5($row->id_laporan)) ?>' 
                                                               style='color:#4defa9' data-target='tooltip' title='Cetak'>
                                                                <i id="<?= md5($row->id_laporan) ?>" class='fa fa-print'></i></a>
                                                            &nbsp;-->
                                                        <!-- <a href="<?= site_url('master_monografi/form/' . md5($row->id_laporan)) ?>" 
